...
func TestSetUseRPC(t *testing.T) {
	...
	rpcServer, err := GetRPCServerProvider()
...
func TestMockRPC(t *testing.T) {
	...
	mockRPCCProvider, mockRPCServerProvider := RegisterAndUseMockRPC(mockCtrl)
	...
	getRPCServer, err := GetRPCServerProvider()
	...
	if getRPCServer != mockRPCServerProvider {
		t.Fatalf("[%v] != [%v], want= equal ", getRPCServer, mockRPCServerProvider)
	}
}
